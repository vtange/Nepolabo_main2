...
#endif
    // MODDED LINE BELOW: Weakens shade color effect on original texture.
    vec3 _col = mix((_shade.rgb + _lit.rgb)*.5, _lit.rgb, _lightIntensity);
...
#endif
    // MODDED LINE BELOW: Boosts lighting while capping it so shadows don't get erased.
    _col *= min(min(_lightColor,vec3(1.,1.,1.)), _lighting * 1.5);
