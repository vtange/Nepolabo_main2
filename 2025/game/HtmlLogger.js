// Copyright (C) 2016 vtange, https://www.vtange.com/.  All rights reserved.
function HtmlLogger() {
    this.open = true;
    ...
    style.innerHTML = `.htmlLoggerLiEntry { border-top:1px solid #777; font-size: 10px; padding-top: 3px; user-select: all;}
    ...
    .dupeCt{background-color:#555;border-radius:4px;padding:0 3px;user-select:none;}`;
...
HtmlLogger.prototype.overrideConsoleLog = function(){
    ...
    var htmlConsoleWrapper = function(originalConsoleFn, iSeverity){
        return function(){
            if(_this.prevArgs && _this.isRepeat(arguments)){
                _this.plusOneForLastLiEntry();
            } else {
                _this.prevArgs = arguments;
                _this.lastLi = _this.addNewBlankLi(iSeverity);
                _this.populateLiEntry(_this.lastLi,arguments);
            }
            originalConsoleFn.apply(_this, arguments);
        };
    }
...
HtmlLogger.prototype.isRepeat = function(arrArgs) {
    for (var i = 0; i < arrArgs.length; i++) {
        if(arrArgs[i] !== this.prevArgs[i]) {
            return false;
        }
    }
    return true;
...
HtmlLogger.prototype.plusOneForLastLiEntry = function() {
    var new_span;
    var dupeCt = this.lastLi.querySelector(".dupeCt");
    if (dupeCt) {
        var currVal = parseInt(dupeCt.innerHTML);
        dupeCt.innerHTML = currVal+1;
    } else {
        new_span = document.createElement("span");
        new_span.className = 'dupeCt';
        new_span.innerHTML = 2;
        this.lastLi.appendChild(new_span);
    }
}
